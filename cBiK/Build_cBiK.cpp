...
void Build_cBiK::load_cBiK(string datasetName) {
    ...
    /**************************************************************************/
    cout << "LOADING HASHMAP >> " << "cBiK_"+datasetName+"_hashmap" << endl;
    ...
    cout << "TOTAL WORDS:" << hashMapKeys.size() << endl;
    this->numKeys = hashMapKeys.size();
    
    /**************************************************************************/
    /************************** CARGAR PUNTOS *******************************/
    /**************************************************************************/
    cout << "LOADING POINTS >> " << "cBiK_"+datasetName+"_points" << endl;
    ...
    /********************** CARGAR BITMAP MAPA ******************************/
    /**************************************************************************/
    cout << "LOADING BITMAP MAP >> " << "cBiK_"+datasetName+"_map.sdsl" << endl;
    load_from_file(mapa, "cBiK_"+datasetName+"_map.sdsl");
    cout << "\t NUMBER OF MAP ELEMENTS >> " << mapa.size()*0.5 << endl;
    ...
    /**************************************************************************/
    cout << "LOADING BITMAP KEYWORDS >> " << "cBiK_"+datasetName+"_keywords.sdsl" << endl;
    load_from_file(sdVectorKeywords, "cBiK_"+datasetName+"_keywords.sdsl");
    cout << "\t NUMBER OF KEYWORDS ELEMENTS >> " << sdVectorKeywords.size() << endl;
    
    /**************************************************************************/
    /********************** CARGAR BITMAP SUMMARY ***************************/
    /**************************************************************************/
    cout << "LOADING BITMAP SUMMARY >> " << "cBiK_"+datasetName+"_summary.sdsl" << endl;
    load_from_file(sdVectorResume, "cBiK_"+datasetName+"_summary.sdsl");
    cout << "\t NUMBER OF SUMMARY ELEMENTS >> " << sdVectorResume.size() << endl;
}
...
bool Build_cBiK::checkKeywords(vector<int> query, vector<int> negQuery, int pos) {
    bool resp = true;
    int i=0;
    ...
    //negative filtering
    while(i<negQuery.size() && resp){
        if(sdVectorKeywords[(posInicial+negQuery[i])] == 1) {
            return false;       //negative keyword found return false
        }
        i++;
    }
...
void Build_cBiK::printKWs() {
    for(int i =0; i <sdVectorKeywords.size(); i++){
        cout << sdVectorKeywords[i] << endl;
    }
}
...
string Build_cBiK::convertNegString(string input){
...
double Build_cBiK::rangeQuery(const double* query, const vector<std::string> &queryKey) {
    ...
    //Basic attributes for the search
    int start = 0;
    int end = nodosKdTree.size() - 1;
    int depth = 0;
    //Key index
    vector<int> indexQueryKey;
    vector<int> indexNegativeQueryKey;
    string str;
    //Variable where the indices to the points that are within the region are stored
    vector<int> result;
    
    //gettimeofday(&iniTime2, NULL);
    //The indexes of the received keywords are searched
    for(int i=0; i<queryKey.size(); i++) {
        //printTuple(query);
        str = queryKey[i];
        str = toLower(str);
        if (str.substr(0, 4) == "neg_"){
            str = convertNegString(str);
            indexNegativeQueryKey.push_back(hashMapKeys[str]);
        } 
        else{    
    ...
    rangeSearchQuery(minPoint, maxPoint, indexQueryKey, indexNegativeQueryKey, start, end, depth, result);
    //gettimeofday(&endTime2, NULL);
...
void Build_cBiK::rangeSearchQuery(const double* query1, const double* query2, vector<int> queryKey, vector<int> negQueryKey, int start, int end, int profundidad, vector<int> &result) {
    ...
    if( (query1[0] <= nodosKdTree[mid].coordenadas[0] && nodosKdTree[mid].coordenadas[0] <= query2[0]) &&
        ...
        if(checkKeywords(queryKey, negQueryKey, mid)) {
    ...
    if(start != end) {
        ...
        if(mayor <= nodosKdTree[mid].coordenadas[p]) {
            ...
            if(checkSummaryLeft(queryKey, mid, start, end)) {
                rangeSearchQuery(query1, query2, queryKey, negQueryKey, start, mid-1, profundidad + 1, result);
            }
            ...
            if(checkSummaryRight(queryKey, mid, start, end)) {
                rangeSearchQuery(query1, query2, queryKey, negQueryKey, mid+1, end, profundidad + 1, result);
            }
        }else {
